...
- Root Gradle must not edit `cpp/lite-rt-lm/Cargo.toml`, `Cargo.lock`, `cargo-bazel-lock.json`, or `WORKSPACE` as a local workaround. Instead, `BuildIosLiteRtLmNativeArchiveTask` runs `bazel sync --only=crate_index`, resolves Bazel `output_base`, and patches the generated `external/crate_index/BUILD.syn-2.0.114.bazel` file with `syn` feature selects for the active `aarch64-apple-ios` and `aarch64-apple-ios-sim` triples before invoking the iOS Bazel build.
