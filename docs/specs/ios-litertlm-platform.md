...
- iOS native build fixes must be applied in the root Gradle wiring and project docs unless the submodule source itself is intentionally being upgraded. Do not patch `cpp/lite-rt-lm/WORKSPACE` or generated lock files as a local build workaround because submodule sync will discard or conflict with those edits.
