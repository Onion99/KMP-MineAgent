...
- Root Gradle must not edit `cpp/lite-rt-lm/Cargo.toml`, `Cargo.lock`, `cargo-bazel-lock.json`, or `WORKSPACE` as a local workaround. Instead, `BuildIosLiteRtLmNativeArchiveTask` runs `bazel sync --only=crate_index`, fetches `@crate_index__syn-2.0.114//:syn`, resolves Bazel `output_base`, and patches the actual per-crate repo `external/crate_index__syn-2.0.114/BUILD.bazel` with `syn` feature selects for the active `aarch64-apple-ios` and `aarch64-apple-ios-sim` triples before invoking the iOS Bazel build. Do not require `external/crate_index/BUILD.syn-2.0.114.bazel` to contain a `crate_features` block; it is not the BUILD file consumed by the iOS rustc action. The generated `syn` rule uses `crate_features = [common] + select({ ... })`; the patch must locate the `select({ ... })` inside the `crate_features` attribute, not `target_compatible_with`. If an iOS platform select already exists in `crate_features`, Gradle must add only the missing feature strings inside that existing list instead of inserting a duplicate dictionary key.
...
- iOS Kotlin/Native linker options include `AVFoundation` because the LiteRT LM audio preprocessing path pulls in miniaudio objects that reference `AVAudioSession`; they also include `AudioToolbox` because miniaudio CoreAudio objects reference `AudioComponent*` and `AudioUnit*` symbols.
- `liblitertlm_c_api.a` must merge the target Rust sysroot `.rlib` files from Bazel's `rust_toolchain/lib/rustlib/<triple>/lib` directory. Crate `.rlib` outputs from `deps(//c:engine)` can still reference Rust `std` symbols such as `std::fs::OpenOptions::_open`, which are not available to the final Kotlin/Native/Xcode link unless the Rust sysroot archives are included.
